...
export class PluginConfigService {
  ...
  async loadPluginConfigSchema(
    ...
    if (pluginPath.startsWith('uploaded://')) {
      ...
      if (!cachedPlugin) {
        throw new Error(`Uploaded plugin ${pluginId} not found in cache`);
      }

      const schemaContent = cachedPlugin.configSchemas?.[manifest.jsonSchemaCfg];
      if (!schemaContent) {
        throw new Error(
          `Config schema ${manifest.jsonSchemaCfg} not found for uploaded plugin ${pluginId}. Reinstall the plugin zip to refresh cached files.`,
        );
      }
